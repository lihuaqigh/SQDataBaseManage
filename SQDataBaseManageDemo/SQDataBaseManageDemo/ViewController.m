...
- (void)addObject {
    ...
    coupon.title = [NSString stringWithFormat:@"更新啦啦商品标题%u",arc4random() % 100];
    coupon.couponId = [NSString stringWithFormat:@"%u",arc4random() % 100 + 100];
    coupon.couponId = [NSString stringWithFormat:@"1000"];
    coupon.timestamp = NSTimeIntervalSince1970;
    coupon.history = YES;
    
//    [SQDataBaseManage addOrUpdataObject:coupon callback:^(BOOL result) {
//        if (result) {
...
- (void)viewDidLoad {
    ...
    NSArray *arr = [SQDataBaseManage objectsWithWhereType:DBWhereTypeHistory page:0 pageSize:100];
    _dataSource = [NSMutableArray arrayWithArray:arr];
    
    self.navigationItem.leftBarButtonItems = @[[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemSearch target:self action:@selector(deleteObjects)]];
